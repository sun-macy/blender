...
// TODO: figure out why using namespace aren't being picked up as keywords
using namespace std;
using namespace rlbox;
using sandbox_type_t = rlbox::rlbox_noop_sandbox;
RLBOX_DEFINE_BASE_TYPES_FOR(jpeglib, noop);

// template<typename T>
// using tainted_img = rlbox::tainted<T, sandbox_type_t>;

// TODO: Define and load any structs needed by the application
// cinfo actually points to a jpegErrorManager struct
// https://stackoverflow.com/questions/19857766/error-handling-in-libjpeg
// I don't know what a jmethod is and it is causing me acute mental distress
#define sandbox_fields_reflection_jpeglib_class_err(f, g, ...)                 \
  f(void, error_exit, FIELD_NORMAL,                                            \
    ##__VA_ARGS__) g() f(void, emit_message, FIELD_NORMAL, ##__VA_ARGS__) g()  \
      f(void, output_message, FIELD_NORMAL, ##__VA_ARGS__) g()                 \
          f(void, format_message, FIELD_NORMAL, ##__VA_ARGS__) g() f(          \
              void, reset_error_mgr, FIELD_NORMAL,                             \
              ##__VA_ARGS__) g() f(int, msg_code, FIELD_NORMAL, ##__VA_ARGS__) \
              g() f(union, msg_parm, FIELD_NORMAL, ##__VA_ARGS__) g()          \
                      ...
                      f(long, num_warnings, FIELD_NORMAL, ##__VA_ARGS__) g()   \
                          f(const char *const *, jpeg_message_table,           \
                            FIELD_NORMAL, ##__VA_ARGS__) g()                   \
                              f(int, last_jpeg_message, FIELD_NORMAL,          \
                                ##__VA_ARGS__) g()                             \
...
static void memory_source(j_decompress_ptr cinfo, const unsigned char *buffer,
                          size_t size);
// this one would be the callback function?
...
static void jpeg_error(j_common_ptr cinfo) {
  ...
  // TODO: sandbox with rlbox
  // actually I don't know if we need to validate this. theoretically we
  // validate cinfo when jpeg_error is originally called so we don't need to do
  // it here? maybe?
  jpeg_destroy(cinfo);
...
static void init_source(j_decompress_ptr cinfo) { (void)cinfo; /* unused */ }
...
static void skip_input_data(j_decompress_ptr cinfo, long num_bytes) {
  ...
  if (num_bytes > 0) {
    ...
    size_t skip_size = (size_t)num_bytes <= src->pub.bytes_in_buffer
                           ? num_bytes
...
static void term_source(j_decompress_ptr cinfo) { (void)cinfo; /* unused */ }
...
ImBuf *imb_load_jpeg(const unsigned char *buffer, size_t size, int flags,
  ...
  rlbox::rlbox_sandbox<sandbox_type_t> sandbox;
  sandbox.create_sandbox();

  // TODO: sandbox with rlbox
  cinfo->err = sandbox.invoke_sandbox_function(jpeg_std_error, &jerr.pub)
  ...
  // TODO: unregister callback functions
...
static void write_jpeg(struct jpeg_compress_struct *cinfo, struct ImBuf *ibuf) {
  ...
  row_pointer[0] = MEM_mallocN(sizeof(JSAMPLE) * cinfo->input_components *
                                   cinfo->image_width,
                               "jpeg row_pointer");
...
static int init_jpeg(FILE *outfile, struct jpeg_compress_struct *cinfo,
                     struct ImBuf *ibuf) {
  int quality;
