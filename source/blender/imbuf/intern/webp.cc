...
// NOTE: webp structs (with janky union workaround)
                                               ...
                                               size_t *r_height)
{
    ...
    *r_width 
        = size_t((tainted_config->input.width).unverified_safe_because("any width is fine"));
    *r_height 
        = size_t((tainted_config->input.height).unverified_safe_because("any height is fine"));
    ...
    rlbox::memcpy(
        sandbox, &(tainted_config->output.u.RGBA.rgba), (uint8_t *)ibuf->rect, sizeof(uint8_t));
...
bool imb_savewebp(struct ImBuf *ibuf, const char *filepath, int /*flags*/) {
    ...
    if (bytesperpixel == 3) {
        ...
        sandbox.free_in_sandbox(tainted_rgb_rect);
        MEM_freeN(rgb_rect);
        ...
        sandbox.free_in_sandbox(tainted_rgba_rect);
    } else {
        fprintf(stderr, 
